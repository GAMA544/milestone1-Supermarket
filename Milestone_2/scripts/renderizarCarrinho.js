...
function alterarQtd(id, delta) {
  ...
  // Recupera a lista completa de produtos do localStorage
  const produtos = JSON.parse(localStorage.getItem("produtos")) || [];
  const produto = produtos.find(p => p.id === id);

  // Se não encontrar o produto ou não houver campo de estoque, assume sem limite
  const estoque = produto ? produto.estoque : Infinity;

  // Verifica se excederia o estoque ao aumentar
...
async function avanzarCompra() {
  ...
  // Verifica se alguma quantidade no carrinho excede o estoque disponível
  for (const item of carrinho) {
    const produto = produtos.find(p => p.id === item.id);
    if (!produto) continue;
    if (item.quantidade > produto.estoque) {
      alert(`A quantidade do produto "${item.nome}" excede o estoque disponível (${produto.estoque}). Por favor, ajuste antes de continuar.`);
      return;
    }
  }
